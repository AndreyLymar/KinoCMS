...
@section('title','Акции - изменение')
@section('link')
    <link rel="stylesheet" href="{{asset('css/styleBannerGallery.css')}}">
@endsection
@section('content')

    <form action="{{route('admin.special_offers.update_or_create')}}" method="post" enctype="multipart/form-data" class="mb-5">
        @csrf
        <input type="hidden" value="{{$special_offer->id}}" name="special_offer_id">
        <div class="row mt-3">
            <div class="col-md-2">
                <label for="title" class="form-label">Название:</label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                       name="title" value="{{old('title') ?? $special_offer->title}}">
                @error('title')
            ...
            <div class="col-md-2">
                ...
                <input type="date" class="form-control @error('date_published') is-invalid @enderror" id="date_published"
                       name="date_published" value="{{old('date_published')  ?? date("Y-m-d", strtotime($special_offer->date_published))}}">
            ...
            <div class="col-md-1 ml-5">
                <div class="form-switch">
                    ...
                    <input class="form-check-input ml-4" type="checkbox" role="switch" id="status" {{$special_offer->status === 1 ? 'checked' : ''}} value = "1" name="status">
                </div>
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-md-6">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror"
                          name="description">{{old('description') ?? $special_offer->description}}</textarea>
                @error('description')
                <span class="invalid-feedback" role="alert">
                        {{ $message }}
                    </span>
        ...
        <div class="row mt-3">
            <div class="col-md-2">
                ...
                <input type="text" class="form-control @error('link_to_video') is-invalid @enderror" id="link_to_video"
                       name="link_to_video" value="{{old('link_to_video') ?? $special_offer->link_to_video}}">
                @error('link_to_video')
                <span class="invalid-feedback" role="alert">
                        {{ $message }}
                    </span>
                    ...
                    <div class="row ">
                        <div class="imageStore position-relative">
                            <img id="imgMain"
                                 ...
                                 src="{{isset($special_offer->main_img) ? Storage::url($special_offer->main_img) : 'https://img.icons8.com/color/96/undefined/downloads.png'}}"/>
                        ...
                        <div class="row">
                            <div class="col-md-4 mt-2 p-0 mr-5">
                                <div class="mb-3 p-0 col-md-12">
                                    ...
                                    <input type="hidden" value="{{$special_offer->main_img}}" id="main_img_old" name="main_img_old">
...
{{--          Галерея картинок     --}}
            ...
            <div class="row ml-3">
                @foreach($special_offer->specialOfferGalleries as $specialOfferGallery)
                <div class="col-2 mt-5 mr-4" id="imgStoreGallery">
                    <div class="row">
                        <div class="imageStore position-relative">
                            <img id="imgO{{$specialOfferGallery->id}}"
                                 class="position-absolute top-50 start-50 translate-middle imgStoreNew"
                                 src="{{Storage::url($specialOfferGallery->img) ?? 'https://img.icons8.com/color/96/undefined/downloads.png'}}"/>
                            <button type="button" onclick="del({{$specialOfferGallery->id}});" class="position-absolute top-10 start-0 translate-middle">
                                <img class="w-50" src="https://img.icons8.com/flat-round/64/undefined/delete-sign.png"/>
                            </button>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12 mt-2 p-0 mr-5">
                            <div class="mb-3 p-0 col-md-12">
                                <label for="formFileO{{$specialOfferGallery->id}}" class="form-label"></label>
                                <input type="hidden" value="{{$specialOfferGallery->id}}" id="idO{{$specialOfferGallery->id}}" name="id[]">
                                <input class="form-control @error('img') is-invalid @enderror " name="img[]"
                                       accept="image/*"
                                       type="file"
                                       onchange="window.checkOld(this.files[0], {{$specialOfferGallery->id}})"
                                       id="formFileO{{$specialOfferGallery->id}}">
                                @error('img')
                                <span class="invalid-feedback" role="alert">
                                        {{ $message }}
                                    </span>
                ...
                @endforeach
                    @for($i = 1; $i <= 5 - count($special_offer->specialOfferGalleries); $i++)
                        <div class="col-2 mt-5 mr-4" id="imgStoreGallery">
                            <div class="row">
                                <div class="imageStore position-relative">
                                    <img id="img{{$i}}"
            ...
            <div class="col-md-8">
                <div class="row mt-3">
                    ...
                    <input type="text" class="form-control @error('seo_url') is-invalid @enderror" id="seo_url"
                           name="seo_url" value="{{old('seo_url') ?? $special_offer->seo_url}}">
                    @error('seo_url')
                    <span class="invalid-feedback" role="alert">
                        {{ $message }}
                    </span>
                ...
                <div class="row mt-3">
                    ...
                    <input type="text" class="form-control @error('seo_title') is-invalid @enderror" id="seo_title"
                           name="seo_title" value="{{old('seo_title') ?? $special_offer->seo_title}}">
                    @error('seo_title')
                    <span class="invalid-feedback" role="alert">
                        {{ $message }}
                    </span>
                ...
                <div class="row mt-3">
                    ...
                    <input type="text" class="form-control @error('seo_keywords') is-invalid @enderror"
                           id="seo_keywords"
                           name="seo_keywords" value="{{old('seo_keywords') ?? $special_offer->seo_keywords}}">
                    @error('seo_keywords')
                    <span class="invalid-feedback" role="alert">
                        {{ $message }}
                    </span>
                ...
                <div class="row mt-3">
                    ...
                    <textarea class="form-control @error('seo_description') is-invalid @enderror"
                              name="seo_description">{{old('seo_description') ?? $special_offer->seo_url}}</textarea>
                </div>
            </div>
        </div>
