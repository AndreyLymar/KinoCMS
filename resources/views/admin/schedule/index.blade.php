    ...
    <div class="row mt-5">
        <table class="table">
            ...
            @foreach($schedules as $schedule)
                @php
                $hall_schedule = \App\Models\Hall::find($schedule->hall_id);
                @endphp
                <tr>
                    ...
                    <td>
                        <select class="form-select cinemas @error('cinema') is-invalid @enderror" data-id="{{$schedule->id}}" name="cinema" aria-label="Default select example">
                            @foreach($cinemas as $cinema)
                                <option value="{{$cinema->id}}" {{old('cinema') ? 'selected' : ($cinema->id == $hall_schedule->cinema_id ? 'selected' : '')}}>{{$cinema->title}}</option>
                            @endforeach
                        </select>
                        @error('cinema')
                        <span class="invalid-feedback" role="alert">
